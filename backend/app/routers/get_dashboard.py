...
def get_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    for reading_book in my_books:
        book = (
            db.query(models.My_book)
            .filter(models.My_book.id == reading_book.id)
            .first()
        )
        total_pages_read = (
            db.query(func.sum(models.Daily_log.page_read))
            .filter(models.Daily_log.my_book_id == book.id)
            .scalar()
            or 0
        )
        ...
        dashboard_list.append(
            {
                ...
                "image": book.image,
                "progress_rate": rounded_progress_rate,
    ...
    reading_date_list = (
        db.query(models.Daily_log.date)
    ...
    award_date_list = (
        db.query(models.User_award.award_date)
